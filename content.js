...
(function() {
  ...
  const SELECTORS = {
    ...
    sidebarAds: [
      '.da-card-creative-click-area',
      '.ad-banner-container',
      '.ads-container',
      '[data-ad-banner]',
      '.ad-banner-card',
      '.artdeco-card:has(.ad-banner)',
      '.ad-banner',
      '.premium-upsell-card'
    ],
    postText: [
  ...
  /**
   * Hide sidebar ad units
   */
  function hideSidebarAds() {
    if (!userRules.hidePromoted) return;
    
    SELECTORS.sidebarAds.forEach(selector => {
      try {
        document.querySelectorAll(selector).forEach(ad => {
          if (!ad.classList.contains('feedhole-hidden-ad')) {
            ad.classList.add('feedhole-hidden-ad');
            ad.style.display = 'none';
            stats.filtered++;
            if (badge) badge.updateStats(stats.filtered, stats.processed);
            console.log('[FeedHole] Hidden sidebar ad:', selector);
          }
        });
      } catch (e) {
        // Selector may not be valid in all browsers (e.g., :has())
      }
    });
  }
  
  /**
   * Watch for dynamically loaded sidebar ads
   */
  function observeSidebarAds() {
    const sidebarObserver = new MutationObserver(() => {
      hideSidebarAds();
    });
    
    // Observe the whole document for sidebar ad injection
